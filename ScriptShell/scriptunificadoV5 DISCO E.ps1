...
# Verificar se o grupo F1SFTP2 existe, se não existir, cria o grupo
if (-not (Get-LocalGroup | Where-Object Name -eq $grupo)) {
    New-LocalGroup -Name $grupo -Description "Grupo para usuários do F1SFTP2"
...
foreach ($usuarioObj in $usuarios) {
    ...
    try {
        ...
        if (Get-LocalUser | Where-Object Name -eq $usuario) {
            ...
            # Adicionar o usuário ao grupo F1SFTP2
